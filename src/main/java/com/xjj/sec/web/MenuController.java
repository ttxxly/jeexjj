...
public class MenuController extends SpringControllerSupport{
	...
	public String list(Model model,
		...
		for (int i = 0; i < menuList.size(); i++) {
			...
			menu = menuList.get(i);
			if (!"system".equals(info.getLoginName())) {
				if ("01".equals(menu.getCode())) {
					removeIndex = i;
				}else if ("02".equals(menu.getCode())) {
					List<MenuEntity> menusByPid = menuService.findMenusByPid(menu.getId());
					menusByPid.removeIf(menuEntity -> "0203".equals(menuEntity.getCode()));
					menu.setSubMenus(menusByPid);
				}else {
					//查询子菜单
